...
def main():
    ...
    if choice == "Home":
        ...
        ''')
        image_path = ("D:\Desktop\Twitter Scrapping Project _main\elonmusktwt.png") # Update path if needed
    ...
    # SEARCH TAB
        ...
        with st.form(key="search_form"):
            ...
            username = st.text_input("Enter Twitter username (without @)").strip().lower()
            ...
            if not username:
                ...
                try:
                    ...
                    if tweets.data:
                        ...
                        # --- Show only this user's tweets immediately ---
                        df_user = pd.read_sql_query(
                            "SELECT * FROM tweets WHERE username = ? ORDER BY date DESC",
                            conn,
                            params=(username,)
                        )
                        st.subheader(f"Showing stored tweets for @{username}")
    ...
    # DISPLAY TAB
        ...
        if st.button("🗑️ Clear All Tweets from Database"):
            c.execute("DELETE FROM tweets")
            conn.commit()
            st.success("All tweets have been deleted from the database.")

        df = pd.read_sql_query("SELECT * FROM tweets ORDER BY date DESC", conn)
        if df.empty:
            st.info("No tweets stored in the database.")
        else:
            st.dataframe(df)
    ...
    # DOWNLOAD TAB
        ...
        if df.empty:
            st.info("No tweets available to download.")
        else:
